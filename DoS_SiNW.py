...
if __name__ == '__main__':
    ...
    E_v = np.linspace(-1.0, 0.2, 300)
    E_c = np.linspace(1.3, 2.0, 300)
    h = 0.01

    dos_c = np.zeros(E_c.shape)
    dos_v = np.zeros(E_v.shape)

    k_new = np.linspace(0, 0.57, 1000, endpoint=True)
    ...
    ax3.set_ylabel(r'DoS [ 1/(eV*nm) ]')
    ax3.set_title('Density of States of Conduction Band')
    fig3.tight_layout()
    ...
    ax4.set_ylabel(r'DoS [ 1/(eV*nm) ]')
    ax4.set_title('Density of States of Valance Band')
    fig4.tight_layout()
